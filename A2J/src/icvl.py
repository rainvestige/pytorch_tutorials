...
testingImageDir = '/media/windows/data/data/ICVL/Testing/processed_seq/'
keypointsfile = '../data/icvl/icvl_keypointsUVD_test.mat'
#keypointsfile = '../data/icvl/keypoints3D_seq_2.mat'
center_file = '../data/icvl/icvl_center_test.mat'
#center_file = '../data/icvl/centre_pixel_seq_2.mat'
...
#keypointsUVD_test = scio.loadmat(keypointsfile)['keypoints3D_seq_2'].astype(np.float32)   

center_test = scio.loadmat(center_file)['centre_pixel'].astype(np.float32)
#center_test = scio.loadmat(center_file)['centre_pixel_seq_2'].astype(np.float32)
...
def dataPreprocess(index, img, keypointsUVD, center, mean, std, lefttop_pixel, 
        rightbottom_pixel, validIndex, xy_thres=95, depth_thres=150):
    ...
    label_xy[:,0] = (keypointsUVD[validIndex[index],:,0].copy() - new_Xmin)*cropWidth/(
                    new_Xmax - new_Xmin) # x
    label_xy[:,1] = (keypointsUVD[validIndex[index],:,1].copy() - new_Ymin)*cropHeight/(
                    new_Ymax - new_Ymin) # y
    
    
    imageOutputs[:,:,0] = imgResize

    labelOutputs[:,1] = label_xy[:,0]  # swap height and width
...
class my_dataloader(torch.utils.data.Dataset):
    ...
    def __init__(self, trainingImageDir, center, lefttop_pixel, 
                 rightbottom_pixel, keypointsUVD, validIndex):
    ...
    def __getitem__(self, index):
        ...
        depth = scio.loadmat(self.trainingImageDir + str(self.validIndex[index]+1) + 
                '.mat')['img']       
         
        data, label = dataPreprocess(
            index, depth, self.keypointsUVD, self.center, self.mean, self.std, 
            self.lefttop_pixel, self.rightbottom_pixel, self.validIndex, 
            self.xy_thres, self.depth_thres)
...
test_image_datasets = my_dataloader(
    testingImageDir, center_test, test_lefttop_pixel, 
    test_rightbottom_pixel, keypointsUVD_test, validIndex_test)

test_dataloaders = torch.utils.data.DataLoader(
    test_image_datasets, batch_size = batch_size, shuffle = False, 
    num_workers = 8)
...
def train():
    ...
    post_precess = anchor.post_process(
        shape=[cropHeight//16,cropWidth//16],stride=16,P_h=None, P_w=None)
    criterion = anchor.A2J_loss(
        shape=[cropHeight//16,cropWidth//16], thres = [16.0,32.0], stride=16,
    ...
    for epoch in range(nepoch):
        ...
        for i, (img, label) in enumerate(train_dataloaders):
            ...
            # printing loss info
            if i%10 == 0:
                print('epoch: ',epoch, ' step: ', i, 'Cls_loss ', Cls_loss.item(),
                      'Reg_loss ',Reg_loss.item(), ' total loss ',loss.item())
